...
def build_admin_telegram_receipt_url(
    *,
    token: str,
    ...
    admin = (settings.payment_admin_telegram or "").strip().lstrip("@")
    if not admin:
        return None
    price = format_price_uzs(amount)
    text = (
        "MBTI premium to'lov qilindi\n"
        f"Sessiya: {token}\n"
        f"Natija: {result_type or '—'}\n"
        f"Summa: {price} so'm"
    )
    return f"https://t.me/{admin}?text={quote(text)}"
...
class PremiumPaymentService:
    ...
    def start_premium_from_deeplink(
        ...
        *,
        ...
        if active:
            ...
            return PremiumStartOutcome(code="existing_active", session=session, payment=active)

        rejected = self.payments.get_latest_rejected_for_session(session.id)
        if rejected:
            rejected.status = PAYMENT_STATUS_PENDING
            rejected.rejected_at = None
            rejected.receipt_file_id = None
            rejected.receipt_type = None
            rejected.receipt_sent_at = None
            self._sync_payment_user(rejected, telegram_user_id, telegram_username, telegram_first_name)
            self.db.commit()
            self.db.refresh(session)
            self.db.refresh(rejected)
            return PremiumStartOutcome(code="reactivated", session=session, payment=rejected)
        ...
        self.db.refresh(session)
        return PremiumStartOutcome(code="created", session=session, payment=payment)
    ...
    def attach_receipt(
        ...
        *,
        ...
        if payment_id is not None:
            ...
            if candidate and candidate.telegram_user_id == telegram_user_id:
                payment = candidate
        if payment is None:
            payment = self.payments.get_latest_awaiting_receipt_for_user(telegram_user_id)
        if payment is None:
            rejected = self._latest_rejected_for_user(telegram_user_id)
            if rejected:
                payment = rejected
        if payment is None:
        ...
        if payment.status not in (PAYMENT_STATUS_PENDING, PAYMENT_STATUS_RECEIPT_SENT, PAYMENT_STATUS_REJECTED):
    ...
    def approve_payment(self, payment_id: int, *, approved_by: str) -> ModerationOutcome:
        payment = self.payments.get_by_id_with_session(payment_id)
        if not payment:
    ...
    def _latest_rejected_for_user(self, telegram_user_id: int) -> PaymentRequest | None:
    ...
    def begin_web_manual_payment(self, session_token: str) -> PaymentRequest | None:
        ...
        tg_id = session.telegram_user_id if session.telegram_user_id is not None else 0
        payment = self.payments.create_payment(
            session_id=session.id,

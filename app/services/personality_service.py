...
class PersonalityService:
    ...
    def get_test_view(self, token: str, question_index: int | None = None) -> dict:
        ...
        questions = self.repo.get_session_questions_ordered(session)
        total = len(questions)
        if total == 0:
    ...
    def submit_answer(
        ...
        *,
        ...
        expected_index = next(i for i, item in enumerate(questions) if item.id == question_id)
        if question_index != expected_index:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid question order")
        ...
        if next_index >= total:
            if not self.repo.all_selected_questions_answered(session):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="All questions must be answered before completing",
                )
    ...
    def prepare_test_start(
        self,
        session: PersonalityTestSession,
        theme: AppearanceTheme,
    ) -> PersonalityTestSession:
        """Bind gender/theme and persist the session question draw before opening question 1."""
        self.repo.set_appearance_theme(session, theme)
        self.repo.get_session_questions_ordered(session)
        self.repo.db.flush()

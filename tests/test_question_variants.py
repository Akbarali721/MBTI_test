...
def test_visitor_gets_questions_from_the_assigned_variant(client, monkeypatch):
    """B to'plamiga tayinlangan foydalanuvchi faqat B bankidan savol oladi."""
    from app.repositories.personality_repository import PersonalityRepository
    ...
    with db_session(client) as db:
        ...
        repo = PersonalityRepository(db)
        selected = repo.get_session_questions_ordered(session)
        assert len(selected) == 24
        assert all(q.variant == "B" for q in selected)
        assert question_id in {q.id for q in selected}
        shown = db.get(PersonalityQuestion, question_id)
        assert shown is not None
        assert shown.variant == "B"

...
from tests.helpers import (
    complete_session,
    db_session,
    favor_left_option_index,
    favor_right_option_index,
    session_by_token,
)
...
def _member_joins(
    ...
    *,
    ...
    with TestClient(app) as member:
        member.testing_session_factory = host_client.testing_session_factory  # type: ignore[attr-defined]
        complete_session(member, option_index=option_index, option_picker=option_picker)
...
def test_member_appears_on_the_dashboard(client):
    manage_code, invite_code = _create_team(client)
    _member_joins(client, invite_code, "Dilnoza")
...
def test_composition_counts_types_and_members(client):
    manage_code, invite_code = _create_team(client)
    _member_joins(client, invite_code, "A", option_picker=favor_left_option_index)
    _member_joins(client, invite_code, "B", option_picker=favor_left_option_index)
    _member_joins(client, invite_code, "C", option_picker=favor_right_option_index)
...
def test_balance_flags_a_skewed_team(client):
    ...
    manage_code, invite_code = _create_team(client)
    for name in ("A", "B", "C", "D"):
        _member_joins(client, invite_code, name, option_picker=favor_left_option_index)

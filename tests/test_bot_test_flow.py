...
def _run_full_test(db, *, option_position: int = 0, option_picker=None) -> test_flow._FinishedView:
    ...
    for _ in range(100):
        if option_picker is not None:
            question = db.get(PersonalityQuestion, view.question_id)
            assert question is not None
            position = option_picker(question)
            option_id = view.option_ids[position]
        else:
...
def test_bot_result_matches_the_web_scoring(client):
    ...
    from tests.helpers import complete_session, favor_left_option_index, session_by_token

    web_token = complete_session(client, option_picker=favor_left_option_index)
    with db_session(client) as db:
        web_type = session_by_token(db, web_token).result_type
        bot_result = _run_full_test(db, option_picker=favor_left_option_index)
...
def test_opposite_answers_give_the_mirrored_type(client):
    ...
    with db_session(client) as db:
        first = _run_full_test(db, option_picker=favor_left_option_index)
        last = _run_full_test(db, option_picker=favor_right_option_index)
